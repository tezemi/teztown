GM.Name = "Trouble in Tez Town"
GM.Author = "Bad King Urgrain, Tezemi"
GM.Website = "ttt.badking.net, tezemi.dev"
...
include("roles_sleeper.lua") -- registers a variant, needs the registry above
include("roles_joker.lua") -- registers a variant, needs the registry above
include("roles_hitman.lua") -- registers a variant, needs the registry above
...
function GM:PlayerFootstep(ply, pos, foot, sound, volume, rf)
   if IsValid(ply) and (ply:Crouching() or ply:GetMaxSpeed() < 150 or ply:IsSpec() or (ply:HasEquipmentItem(EQUIP_HUSH))) then
